//! FGP daemon for Fly.io operations.
//!
//! Uses Fly.io GraphQL API directly for low-latency operations.
//!
//! # Usage
//! ```bash
//! fgp-fly start           # Start daemon in background
//! fgp-fly start -f        # Start in foreground
//! fgp-fly stop            # Stop daemon
//! fgp-fly status          # Check daemon status
//! ```
...
use crate::service::FlyService;

const DEFAULT_SOCKET: &str = "~/.fgp/services/fly/daemon.sock";

#[derive(Parser)]
#[command(name = "fgp-fly")]
#[command(about = "FGP daemon for Fly.io operations")]
#[command(version)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    Start {
        /// Socket path (default: ~/.fgp/services/fly/daemon.sock)
...
fn cmd_start(socket: String, foreground: bool) -> Result<()> {
    ...
    // Get token BEFORE fork (env access needs parent process)
    let token = std::env::var("FLY_ACCESS_TOKEN")
        .context("FLY_ACCESS_TOKEN environment variable not set")?;

    let pid_file = format!("{}.pid", socket_path);

    println!("Starting fgp-fly daemon...");
...
fn cmd_stop(socket: String) -> Result<()> {
    ...
    println!("Stopping fgp-fly daemon (PID: {})...", pid);
